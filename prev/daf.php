...
$result=mysql_query("select * from pixel_registration where ID ='$userid'",$con);
	while($row=mysql_fetch_array($result))
	{
		...
		{ $userid;
		 $level=$row['COUNT'];
		 $pass=strtolower($row['DOB']);
		}
	}
if(($_SESSION['userid']=="")||($level!= 18)){////// 1>LEVEL NEED TO BE CHANGED/////if level 1 then write 0
head(0);
 exit;
}else{
...
<div class="balloon" >
	<div class="arrow" ></div>
	<img src="img/riddle/cong.png"/><!---change img--->
...
<div class="balloon2" >
    ...
    <form method="post" action="daf.php"><!---<<< 2 >>> change the action---->
...
if(isset($_POST['answer'])){		
	...
	if($answer1== strtolower($userid)) /////////////// <<<<3>>>>CHANGE THE ANSWER FOR THE question
	{	
		$count=19;  
		$_SESSION['level']=19;	//////// change the count to relevant level
