<?php
	require_once('conf/mysql_init.php');
	 ...
	 $result=mysql_query($query,$con);
		...
		$nameErr = $emailErr = $pwdErr = $repwdErr = $emailAlreadyErr = $contactnoErr = "" ;
		$name = $email = $password = $repwd = $contactno = "";
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST") {
		  ...
		  if (empty($_POST["gendersignup"])) {
			$genderErr = "Required";
		  } else {
			$gender = test_input($_POST["gendersignup"]);
		  ...
		  if (empty($_POST["locationsignup"])) {
			$locationErr = "Location required";
		  } else {
			$location = test_input($_POST["locationsignup"]);

		  }
		  if (empty($_POST["accsignup"])) {
			$accErr = "Required";
		  } else {
			$acc = test_input($_POST["accsignup"]);
	...
	if($_SERVER["REQUEST_METHOD"] == "POST" &&(empty($nameErr)) && (empty($emailErr)) && (empty($pwdErr)) && (empty($repwdErr)) && (empty($contactnoErr)) && (empty($genderErr)) && (empty($colgErr)) && (empty($locationErr)) && (empty($accErr))) {
		...
		$emailErr = "shit";
		$sql1="INSERT INTO `pixel_test`.`test_16` (`firstname`, `pixelid`, `password`, `mailid`, `contactno`,`gender`,`colg`,`location`,`acc`,`count`) VALUES ('$name', '$id', '$password', '$email','$contactno','$gender','$colg','$location','$acc','0');";
    ...
    <body>
		...
		<div class="container">
			<section class="content">
				...
				<input class="input__field--kaede" name="gendersignup" required="required" type="button" value="xxx" id="input-6" maxlength="30"/>
						<label class="input__label input__label--kaede" for="input-6">
							<span class="input__label-content input__label-content--kaede">Gender</span>
						</label>
						<script>
    			...
    			$("#xxx").click(function(){
				 $("#l").show();
					...
					<span class="input input--kaede">
						...
						<input class="input__field--kaede" name="colgsignup" required="required" type="text" id="input-7" maxlength="30"/>
						<label class="input__label input__label--kaede" for="input-7">
							<span class="input__label-content input__label-content--kaede">College Name</span>
						</label>
					</span>
					<span class="input input--kaede">

						<input class="input__field--kaede" name="locationsignup" required="required" type="text" id="input-8" maxlength="30"/>
						<label class="input__label input__label--kaede" for="input-8">
							<span class="input__label-content input__label-content--kaede">Location</span>
						</label>
					</span>
					<span class="input input--kaede">

						<input class="input__field--kaede" name="accsignup" required="required" type="text" id="input-9" maxlength="30"/>
						<label class="input__label input__label--kaede" for="input-9">
							<span class="input__label-content input__label-content--kaede">Accommodation</span>
						</label>
					</span>
					<span class="input input--kaede">
